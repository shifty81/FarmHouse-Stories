...
@export var growth_day_thresholds := {
	Stage.AWAKENING: 21,
	Stage.BUDDING: 35,
	Stage.BLOOMING: 50,
	Stage.RADIANT: 70,
...
func check_growth_stage() -> void:
	...
	for stage in [Stage.SANCTUARY, Stage.RADIANT, Stage.BLOOMING, Stage.BUDDING, Stage.AWAKENING]:
...
func show_tree_info() -> void:
	...
	print("Echo Tree [", stage_name, "]: ", description)
	
	# TODO: Show dialogue UI with tree lore
	if current_stage >= Stage.RADIANT:
		print("The tree hums with ancient energy. You can feel your connection to the land deepening.")
	elif current_stage >= Stage.AWAKENING:
		print("The tree is responding to your care. Keep restoring the farm.")
	else:
		print("The tree is dormant, waiting for the land to heal.")
